...
I've attempted to reproduce the results from Sadinle's 2017 paper, which has been a little bit of a mixed bag. The code we have does not seem to be the same that is used in the actual 2017 paper. I think this for two reasons:

- Code involves three datasets (ER, CHDES, and UNTC), while paper only links two (ER and CHDES)
- There is code for priors for "truncation points" which are elements of Sadinle's 2014 model.

Nevertheless, I proceeded. For a first run-through, I tried to build the comparison vectors in the exact same way that Sadinle did. One difference in implementation is that his paper calls for three agreement levels for location of death (exact municipality, adjacent municipality, nonmatch), but the code I have doesn't do that, and that would be pretty difficult to implement. So I just use binary comparisons.
...
- The paper has a Bayes estimate with 187 links, but my implementation (with `BRL`) finds 179. This might be because the paper detects adjacent municipalities, while I do not.
...
Below is the posterior distribution of the overlap between the files. We see that `parlr` typically matches more, which is to be expected; at minimum, there are the 13 matches that `parlr` made that violated one-to-one matching which Sadinle avoids. For two reasons, I do not consider this to be a problem.
...
- Severe overmatching within the Gibbs iterations can possibly lead to biased posteriors for the $m$ parameters (this is Sadinle's critique of Fellegi Sunter). This led to me worry that parameter estimates under `parlr` would be similarly biased. However, good news, they are not! This is because the overmatching under `parlr` is very slight compared to FS and `fastlink`.

<!-- When Enamorado compared `fastlink` to `BRL`, he notes that the first estimate under `fastlink` has way too many matches, but that `fastlink` produces similar results to `BRL` after resolving all the matches that violate one-to-one. While this is true, the overmatching within his EM algorithm lead to biases in the $m$ parameters, and these can not be cleaned up posthoc. In the Sadinle paper, he notes that the FS method leads to nonsensical estimates of the $m$ parameters, and I believe it is for this reason. -->
