...
\begin{align}
	\mathcal{L}(Z_j\mid  \tilde{\gamma}, \bm{m}, \bm{u}, \pi) = \prod_{p=1}^P u_p^{N_{p_j}} \prod_{i \in r_{p_j}} w_p^{I(Z_j = i)}. \label{likelihood_efficient_z}
\end{align}
With this form, we break this sampling step into two. We first sample among $P + 1$ options for the agreement pattern between $B_j$ and its potential link. Define $r$ as an arbitrary set of records. We have
\begin{align}
	\label{eqn:gibbs1}
...
As shown in the full conditionals in (\ref{efficient_alpha}),  (\ref{efficient_beta}), and (\ref{eqn:gibbs1}), all original record pairs are still accounted for through $\mathcal{N}$. We must revise (\ref{eqn:gibbs2}) to account for the reduced number of labels, so instead use
\begin{align}
