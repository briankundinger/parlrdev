...
Lastly, we note that it is common to encounter missing information in datasets used for record linkage. Whether information is missing in record $i \in \bf{X}_1$ or $j \in \bf{X}_2$, the comparison vector $\bf{\gamma}_{ij}$ will have missing values. We handle this issue by assuming that this missingness occurs at random (MAR, by \cite{LittleRubin2002})
...
Since $\pi$ encodes the rate of matching across the two datasets, the posterior distribution $p(\pi|\Gamma, \bf{Z}, \Phi, \alpha_{\pi}, \beta_{\pi})$ depends only on the number of links $n_{12}(\bm{Z}) = \sum_{i=1}^{n_2}I(z_j < n_1 + j)$ encoded by $\bm{Z}$ (and hyperparameters). Thus we use the abbreviated $p(\pi | \bf{Z}, \alpha_{\pi}, \beta_{\pi})$, and have 

%As a function of \(\pi\), \(\bm{Z}\) is sequence of successes (when \(z_j < n_1 + j\)) and failures (when \(z_j = n_1 + j\)), and therefore \(p(\bm{Z}|\pi) = \mathcal{L}(\pi|\bm{Z})\) is determined only by the number of links $n_{12}(\bm{Z}) = \sum_{i=1}^{n_2}I(z_j < n_1 + j)$ encoded by $\bm{Z}$. We have
...
\textcolor{red}{I added the second line, showing the division by all the u's. Its pretty ugly! What do you guys think?}
...
\textcolor{red}{This is the derivation for integrating out $\pi$. Should it go here, or in the appendix?}
\begin{align*}
	\int_{\pi}p(Z_j | \Phi, \Gamma, \pi) d\pi &= \int_{\pi}\left[\frac{\pi}{n_1}w_{z_{j}, j} I(z_j\leq n_1) + (1-\pi)I(z_j = n_1 + j)\right]d\pi \\
	&= \frac{\int_{\pi} \pi d\pi}{n_1}w_{z_{j}, j} I(z_j\leq n_1) + \left(1-\int_{\pi} \pi d\pi \right) I(z_j = n_1 + j) \\
	&= \frac{n_{12}(\bf{Z}) + \alpha_{\pi}}{n_1 (n_2 + \alpha_{\pi} + \beta_{\pi})}w_{z_{j}, j} I(z_j\leq n_1) + \frac{n_2 - n_{12}(\bf{Z}) + \beta_{\pi}}{n_2 + \alpha_{\pi} + \beta_{\pi}}I(z_j = n_1 + j) \\
