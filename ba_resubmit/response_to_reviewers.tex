...
\reply{%
	We thank the Editor, the AE, and the reviewer for their feedback. We have made an effort to make it more clear our contributions in this work as opposed to previous work. Furthermore, we have carefully edited the paper to address issues that were pointed out by the Editorial Board.
	
		\begin{enumerate}
		\item \textcolor{red}{We have included a discussion of our missingness assumptions and the definition of $I_{obs}()$ near the end of Section 3.}
		
		Additionally, we have defined $n_{12}(Z)$ at its first occurance.
		\item We have fixed the inconsistency, using $n_1$ and $n_2$ throughout the paper.
		\item The previous expression $Z_j, \hat{Z}_j \leq n_1$ was meant to be understood as $Z_j\leq n_1, \hat{Z}_j \leq n_1$ We agree that the latter expression is more clear, and have made that change.
		\item  We have updated the Lemma to include the definition of $B$, in addition to all other variables included in the Lemma. 
		\item \textcolor{red}{BEKA/JERRY: This exact reference seems purely stylistic. We need to decide how to respond to this.}
	\end{enumerate}
}
...
\reply{%
	\textcolor{red}{We have included a discussion of our missingness assumptions and the definition of $I_{obs}()$ near the end of Section 3.}
}
...
\reply{%
	We have changed $w_{ij}$ to $w_{z_j, j}$ in the pmf for $\Gamma_{.j}$ and the full conditional for $Z_j$. We have been sure that “$n_1+1$” has been changed to “$n_1+ j$” throughout the paper. 
}
...
\reply{%
	\textcolor{red}{We have revised the presentation of the Gibbs sampler based on this feedback. The main body of the paper presents the results, and revised derivations (including justification for integrating out $\pi$) have been placed in the appendix.}
}
...
\reply{%
	We have revised the lemma to include definitions for all variables used within the statement. 
}
