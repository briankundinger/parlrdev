...
\reply{%
	\textbf{Our missingness assumptions and the definition of $I_{obs}()$ are near the end of Section 3.}
}
...
\reply{%
	...
	For (4), we have included the following sentence in the paper ``In practice, we recommend $S=10$, as this reduces the number of stored indices for highly unlikely record pairings, but does is not likely to eliminate any of the indices for likely matches."
...
\reply{%
	We have updated the Figure 4 with these results. Since its is presented along side NPV and PPV, for which values close to 1 indicate strong performance, we have presented decision rate (DR) results.
}
