...
\textcolor{red}{Note that 	$\gamma_{Z_j, j}$ doesn't appear to be defined.}
...
\textcolor{teal}{Observe that we compute the vectorized summations in (\ref{efficient_alpha}) and (\ref{efficient_beta}) more efficiently than the previous updates in (\ref{efficient_alpha}) and (\ref{efficient_beta}), which require summing over $n_A n_B$ record pairs for each field and agreement level.}
