...
\end{align}
\end{subequations}
\textcolor{teal}{where $\alpha_{\pi}, \beta_{\pi}$ are fixed hyper-parameters.} 
...
\textcolor{teal}{We present a Gibbs sampler to approximate the joint posterior distribution of $\bm{Z}, \bm{m}, \bm{u}$ given the observed comparison data from the likelihood and priors assumed in (\ref{eqn:likelihood} - \ref{eqn:pi}). While it is easy to marginalize over $\pi$ and derive a collapsed Gibbs sampler, the sampler is sequential in nature as is that of \cite{}. Thus, we proceed with a traditional Gibbs sampler, where we show in Appendix \ref{}, the updates are parallel, making our approach scalable.}
...
In fact, to our knowledge, our approach and that of \cite{wortman2019} are the only two Bayesian approaches that propose parallel updates to the matching label (or linkage structure), contrasting the Bayesian record linkage literature \citep{tancredi2011hierarchical, sadinle_detecting_2014, steorts_bayesian_2016, marchant_distributed_2019, zanella_flexible_2016, betancourt2022prior, aleshinguendel2021multifile}. See Appendix~\ref{app:derivations} for derivations of the full conditional distributions.}
...
\textcolor{red}{Note: Please use s, s+1 consistently throughout. Or supress the notation and tell the reader. It looks weird to go back and forth. I've tried to provide an intermediate solution. Perhaps Jerry can state if he would think this is okay as an Editor.}\\
