...
\textcolor{blue}{See suggestions below in teal color.}	
	
Thank you for the thoughtful feedback. We have made several broad changes to address \textcolor{teal}{the Editor's and Associate Editor's} overall critiques, and have addressed each specific comment from the reviewer.
...
\pointRaised{R}{%
	...
	``Those comparison vectors" referr to removing the larger (and expensive) $\Gamma^{ab}$ from memory and continuing our calculations with the compressed comparison vectors $\tilde{\Gamma}^{ab}.$ 
	
	The revised text states: ``Then, we conduct hashing, obtain the compressed comparison vectors $\tilde{\gamma}^{ab}$, and remove the memory-intensive comparison vectors $\gamma^{ab}$ before continuing with the next chunk of data. See Section 4.3, page 10. 
	}
...
\pointRaised{R}{%
	...
	\textcolor{red}{I'm checking over this. This probably needs to be explained in more detail. Go check very carefully.}
