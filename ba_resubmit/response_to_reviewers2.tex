...
\pointRaised{R}{%
	...
	\reply{Yes, in the original manuscript, the comparison vectors are created and therefore, patterns are assigned) without regard to $Z$. Given the confusion caused to the Associate Editor, we removed the confusing sentence.	}
...
\pointRaised{R}{%
	...
	``Those comparison vectors" refer to removing the larger (and expensive) $\Gamma^{ab}$ from memory and continuing our calculations with the compressed comparison vectors $\tilde{\Gamma}^{ab}.$ 
