...
**For example, $I_obs()$ is never defined, $n_12(Z)$ first appears on p. 5, but is not defined (in passing) until the bottom of p. 7, and it seems to become D on p. 8.**
\begin{quote}
\textcolor{red}{How and when should I define $I_{obs}$? I probably need to lay out certain assumptions about missingness?}
...
**The loss function on p. 9 is out of Sadinle’s paper and it inherits the typo therein $(\theta_{11},$ $if Z_j, \hat{Z_j}, …)$.**
\begin{quote}
Thank you for addressing this oversight. The previous expression $Z_j, \hat{Z}_j \leq n_1$ was meant to be understood as $Z_j\leq n_1, \hat{Z}_j \leq n_1$ We agree that the latter expression is more clear, and have made that change.
\end{quote}
...
\begin{quote}
\textcolor{red}{Is this true? I don't mind making the change, I've just never seen commas in that way.}
\end{quote}
...
\begin{quote}
\textcolor{red}{Please see comment in Reviewer 2 responses.}
\end{quote}
...
\begin{quote}
\textcolor{red}{I can't find this typo}
\end{quote}
...
\begin{quote}
\textcolor{red}{I don't think we require an appendix. It's not much more notation than Sadinle or Fellegi Sunter.}
\end{quote}

**Section 2, line 1: It may help to explicitly define X1 and X2 as being vectors of indices as impliedby equations (1) and (2).**
\begin{quote}
...
\begin{quote}
\textcolor{red}{PLEASE READ: Part of me thinks we should just remove this element of the model from the paper. This topic also begs the question of whether linkages stay the same depending on which dataset you label X1 and X2. I have no good way of answering that.}

\textcolor{red}{Essentially though,the answer is yes, the algorithm can easily be adapted for the reverse sizing. The issue is mostly notational.}

\textcolor{red}{It seems as though Sadinle chose to label the datasets $X_1$ and $X_2$ such that $n_1 > n_2$ for mostly notational reasons. He uses the notation $Z_j = n_1 + j$ when record $j \in X_1$ has no match in $X_2$, which works because when $n_1 > n_2$, $n_1 + j$ never refers to any record in $X_2$. Jody uses the simplified notation $Z_j = n_1 + 1$ to refer to unmatched records, and this works for the same reason. }

\textcolor{red}{However, when we relax the one-to-one constraint, or even encourage a modeller to allow multiple linkages in one dataset, we shouldn't denote the datasets according to simple notational convenience. This might require clever edits of the "Review of Prior Work" section, and other areas of the paper.}

\textcolor{red}{Notationally, we can use  $Z_j = 0$ or $Z_j = -j$ as a way to unambiguously denote a record as nonmatching.}

\textcolor{red}{But again, it may be easier to just strike the paragraph about the advantages of many-to-one matchings.}
\end{quote}

**Section 3, full model: $I_obs(.)$ seems to be taken from a different paper dealing with missing fields, which is not at all discussed in this manuscript. This consideration of missing fields and the accompanying discussion of missing data mechanism should be included if such data is to be considered for this manuscript. This comment also applies to the $Gamma^{obs}$ notation in the following subsection.**
...
\begin{quote}
\textcolor{red}{I'm not entirely sure how to address the first part of the response. Should I just eliminate a lot of the exposition? Also, I see no issue with $p(\Gamma_{.j})$}
\end{quote}
...
\begin{quote}
\textcolor{red}{Ask Beka if $i$ or $z_j$ is more clear.}
...
\begin{quote}
\textcolor{red}{Should I just remove this step? I'm not sure how to justify this step, but also this step isn't actually necessary. We could just sample $\pi$ and use it, as in a standard Gibbs sampler. The only reason to integrate it out is to see the similarity to Sadinle's full conditional, but its not necessary at all.}
\end{quote}
...
\begin{quote}\textcolor{red}{No, the time savings would be the same. In all cases, the fabl method replaces dependency on $n_2$ for dependency on $P$, the number of patterns. To me, this is clear, I'm not sure how to clarify this in the paper}
\end{quote}
...
\begin{quote}
\textcolor{red}{I'm not sure how to address this.}
\end{quote}
