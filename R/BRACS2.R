BRACS_BK2 <- function(comparisons, m_prior, u_prior,
  ...
  varnames <- str_c("X", 1:dim(indicators_raw)[2])
  names(indicators[1:dim(indicators_raw)[2], ]) <- varnames
  indicators <- indicators %>%
    rename_with(.fn = ~stringr::str_replace(., "X", "V"),
                .cols = varying_fields)
  ...
  for(s in 1:S){
    ...
    lc_counts <- indicators %>%
      select(contains("V"), Z.temp, linkage_cluster) %>%
      group_by(linkage_cluster, Z.temp, .drop = FALSE) %>%
      summarise(across(.cols = contains("V"),
                       .fns = sum),
                .groups = "drop")
...
NEW_LC_METHOD <- function(comparisons, m_prior, u_prior,
                      alpha, beta, S, burn, show_progress = T){
  # Implements bipartite record linkage with BK Sampling Mechanism
  #
  # Arguments
  ...
  record_cluster <- rep(0, n1*n2)
  for(k in 1:(K-1)){
    to_change <- which(ids[, 2] %in% n2_indices[[k]])
    record_cluster[to_change] <- k
  }

  n_sizes <- as.vector(sapply(1:(K-1), function(x){
    c(n1_vec[x], n1 - n1_vec[x])
  }))
  #indicators <- data.frame(indicators, linkage_cluster)
  ...
  indicators <- data.frame(indicators_raw, linkage_cluster, record_cluster,Z.temp)
  varnames <- str_c("X", 1:dim(indicators_raw)[2])
  names(indicators[1:dim(indicators_raw)[2], ]) <- varnames
  indicators <- indicators %>%
    rename_with(.fn = ~stringr::str_replace(., "X", "V"),
  ...
  Dvec_id <- indicators %>%
    mutate(Dvec_id = case_when(
      record_cluster == linkage_cluster ~ 2 * (record_cluster - 1) + 1,
      record_cluster != linkage_cluster ~ 2 * record_cluster
    )) %>%
    select(Dvec_id) %>%
    unlist()

  linkage_cluster <- as.numeric(linkage_cluster)

  L_vec <- rep(0, (K-1)*2)
  den <- rep(n2_vec, K-1) - L_vec + beta
  offsets <- 1/n_sizes * (L_vec + alpha)/(den)
  offsets <- offsets[Dvec_id]
  ...
  for(s in 1:S){
    ...
    #weights <- exp((rowSums(logm * indicators_raw) - rowSums(logu * indicators_raw)))
    weights <- exp((rowSums(ratio * indicators_raw)))
    weights <- offsets * weights
    weights <- split(weights, ids[,2])
    #offset <- n1*(n2 - L + beta)/(L + alpha)

    Z <- unname(sapply(weights, function(x){
      sample(c(candidates, n1 + 1), 1, prob = c(x, 1))
    }))
    Z.temp <- factor(as.vector(sapply(Z, function(x){
      if(x < n1 + 1){
        vec <- rep(0, n1)
    ...
    L_vec <- sapply(1:(K-1), function(x){
      c(sum(Z[n2_indices[[x]]] %in% n1_indices[[x]]),
        sum(Z[n2_indices[[x]]] %in% unlist(n1_indices[[-x]])))
    })

    Lsum <- colSums(L_vec) %>%
      rep(., each = 2)

    den <- rep(n2_vec, K-1) - Lsum + beta


    L_vec <- as.vector(L_vec)
    offsets <- 1/n_sizes * (L_vec + alpha)/(den)
    offsets <- offsets[Dvec_id]
