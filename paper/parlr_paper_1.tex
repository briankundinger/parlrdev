...
duplicate records across multiple data sources. This task is
straightforward when provided unique identifiers or highly reliable
information, but becomes difficult when data sources are plagued with
error. Additionally, many entity resolution tasks are large in nature,
and many existing methods are too computational intensive to be used in
practice. In this paper, we propose a Bayesian model for record linkage
that uses hashing techniques and efficient storage mechanisms, and is
amenable to parallelization, making it scalable to large linkage tasks.

The proposed method is inspired by the \emph{Beta Record Linkage}
(\texttt{BRL}) method proposed in Mauricio Sadinle's 2017 ``Bayesian
Estimation of Bipartite Matching for Record Linkage''. To recognize this
lineage, we name our method \emph{Fast Beta Linkage} (\texttt{fabl},
pronounced ``fable''). In what follows, Section 2 describes previous
work in record linkage that lays the foundation for our method, Section
...
Salvadoran casuality records. Lastly, our discussion highlights open
questions in the record linkage liturature and motivates future work.
...
One key strength of Sadinle's \texttt{BRL} is that it is garunteed to
produce a set of bipartite matchings that respect one-to-one
assumptions--without the need for any post-hoc procedures. Specifically,
in each iteration of his Gibbs sampler, he considers each record
\(j\in B\), removes from consideration the records \(i\in A\) that have
...
{\centering \includegraphics[width=29.17in]{../notes/figures/sadinle_sim_plot} 

}

\caption{Posterior means and credible intervals for accuracy metrics under the replication of simulation study from Sadinle 2017. For each level of overlap and each level of error, we have 100 paired sets of 500 records.}\label{fig:unnamed-chunk-1}
\end{figure}
...
{\centering \includegraphics[width=17.75in]{../notes/figures/sadinle_speed_plot2} 

}

\caption{Runtime for BRL and fabl to run 1000 Gibbs iterations, including hashing step for fabl, for increasing values of both $n_A$ and $n_B$}\label{fig:unnamed-chunk-2}
\end{figure}
...
{\centering \includegraphics[width=29.17in]{../notes/figures/speed_plot_fixed_nB} 

}

\caption{Runtime for BRL and fabl to run 1000 Gibbs iterations, including hashing step for fabl, with increasing $n_A$, and $n_B$ fixed at 500}\label{fig:unnamed-chunk-3}
\end{figure}
...
{\centering \includegraphics[width=22.86in]{../notes/figures/el_salvador/m_posterior_smallP} 

}

\caption{Posterior estimates of m parameters with 95\% credible intervals}\label{fig:unnamed-chunk-4}
\end{figure}
...
\hypertarget{violations-of-one-to-one-matching}{%
\subsection{Violations of One-to-One
Matching}\label{violations-of-one-to-one-matching}}

Figure XX shows the posterior distribution for \(D\), the number of
...
{\centering \includegraphics[width=22.86in]{../notes/figures/el_salvador/overlap_distribution_smallP_bayes} 

}

\caption{Posterior distribution of overlap across the two files. The solid lines show the Bayes estimate for the amonut of overlap, and the dashed line is the Bayes estimate under fabl before resolving violations of one-to-one matching}\label{fig:unnamed-chunk-5}
\end{figure}
...
{\centering \includegraphics[width=6.51in]{../notes/figures/el_salvador/rosa_records} 

}

\caption{Example of linkage situation with multiple plausible matches}\label{fig:unnamed-chunk-6}
\end{figure}

\begin{figure}

{\centering \includegraphics[width=22.86in]{../notes/figures/el_salvador/bad_mixing} 

}

\caption{Gibbs sampling in situation with multiple plausible matches.}\label{fig:unnamed-chunk-7}
\end{figure}
...
\texttt{fabl} allows the modeller to inspect records with mulitple
