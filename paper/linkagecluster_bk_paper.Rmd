...
The prior for $Z_j$ has equal probability of matching to all records $i\in \mathbf{X}_A$, and non-matching probability governed by $\lambda$. Observe two reasonable choices for $p(Z, \lambda)$:
...
Assign each record in both files to a linkage cluster $S_k \in \{S_1, \ldots, S_K\}$ based on some attribute in the data. We see that the comparison vector 

<!-- The classic Fellegi Sunter model operates through weights $w_{ij} = \frac{P(\boldsymbol{\gamma}_{ij}| \text{Records match})}{P(\boldsymbol{\gamma}_{ij}| \text{Records do not match})}$. In our notation, these weights are given as follows: -->
<!-- $$w_{ij} = \frac{P(\boldsymbol{\gamma}_{ij}| Z_j = i)}{P(\boldsymbol{\gamma}_{ij}| Z_j \neq i)} = \frac{\prod_{f=1}^F \prod_{l=1}^{L_f} m_{fl}^{\mathbf{1}_{\gamma_{ij}^f = l}}}{\prod_{f=1}^F \prod_{l=1}^{L_f} u_{fl}^{\mathbf{1}_{\gamma_{ij}^f = l}}}$$ -->
...
<!-- $$p(\Gamma_{.j}|Z_{j}) = \frac{\mathbf{1}_{z_j < n_A + 1}\lambda\prod_{i=1}^{n_A}\prod_{f=1}^{F}\prod_{l=1}^{L_f} \left(m_{fl}^{\mathbf{1}_{z_j = i}}u_{fl}^{\mathbf{1}_{z_j \neq i}}\right)^{\mathbf{1}_{\gamma_{ij}^f = l}} + \mathbf{1}_{z_j = n_A + 1}(1-\lambda)\prod_{i=1}^{n_A}\prod_{f=1}^{F}\prod_{l=1}^{L_f} u_{fl}^{\mathbf{1}_{\gamma_{ij}^f = l}}}{\sum_{z_j = i}^{n_A}\mathbf{1}_{z_j < n_A + 1}\lambda\prod_{i=1}^{n_A}\prod_{f=1}^{F}\prod_{l=1}^{L_f} \left(m_{fl}^{\mathbf{1}_{z_j = i}}u_{fl}^{\mathbf{1}_{z_j \neq i}}\right)^{\mathbf{1}_{\gamma_{ij}^f = l}} + \mathbf{1}_{z_j = n_A +1}(1-\lambda)\prod_{i=1}^{n_A}\prod_{f=1}^{F}\prod_{l=1}^{L_f} u_{fl}^{\mathbf{1}_{\gamma_{ij}^f = l}}}$$ -->
<!-- We can divide the numerator and denominator by $\prod_{i=1}^{n_A}\prod_{f=1}^{F}\prod_{l=1}^{L_f} u_{fl}^{\mathbf{1}_{\gamma_{ij}^f = l}}$ to obtain -->

<!-- $$p(\Gamma_{.j}|Z_{j}) =\frac{\mathbf{1}_{z_j < n_A + 1}\lambda\prod_{i=1}^{n_A}\prod_{f=1}^{F}\prod_{l=1}^{L_f} \left(\frac{m_{fl}}{u_{fl}}\right)^{\mathbf{1}_{z_j = i} \times \mathbf{1}_{\gamma_{ij}^f = l}} + \mathbf{1}_{z_j = n_A + 1}(1-\lambda)}{\sum_{z_j = 1}^{n_A}\mathbf{1}_{z_j < n_A + 1}\lambda\prod_{i=1}^{n_A}\prod_{f=1}^{F}\prod_{l=1}^{L_f} \left(\frac{m_{fl}}{u_{fl}}\right)^{\mathbf{1}_{z_j = i} \times \mathbf{1}_{\gamma_{ij}^f = l}} + \mathbf{1}_{z_j = n_A + 1}(1-\lambda)}$$ -->


<!-- Our model decouples the draws of the elements of the linkage structure, sampling each $Z_j$ independently from all other $Z_{j'}$. This decoupling allows the incorporation of additional information on each record $j$, thus enabling the linkage cluster formulation proposed in Section XXXX. We clearly admit that this does allow the possibility that $Z_j = i = Z_{j'}$ for some $i\in \mathbf{X}_A$, breaking one-to-one matching requirements in individual Gibbs iterations. However, if our model assumptions are correct and there are no duplicates within files, these sorts of illegal matches are rare and evenly distrubted across records, and thus do not seriously influence our eventual Bayes estimate of the linkage structure. Additionally, these illegal matches can easily be resolved in a post-hoc conflict resolution step, or "recoupling", as described in Section XXXX. -->
