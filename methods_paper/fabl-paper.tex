...
\textcolor{blue}{For Beka: The simulated files contain 6 fields of information, but he only uses 4 in his study. One of the fields was giving me problems, so I had been using a different one instead. I just figured out how to make the field work, so I can rerun the code. At that point, it will be a 100\% true replication.}
...
{
\color{blue}For Beka: Yes, I used Table 2 (those are default settings in BRL)
	
My Figure 1 is equivalent to Sadinle's figure 3. He compares against Fellegi Sunter, whereas I compare against BRL. I found his style of graphic difficult to read, especcially because I wanted to show F-measure in addition to recall and precision. 

Sadinle's Figure 4 just shows the results when you add the option of not declaring matching status (leaving it undecided by the model). This didn't seem to be so relevant in my paper. I can make the plot by changing just a few lines of code, so I can do it if you think I need it!}
