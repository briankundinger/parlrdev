...
{
\color{blue}For Beka: I have re-run the simulation exactly as Sadinle implemented it. Here is the rewritten paragraph.
...
{
\color{blue}For Beka: Yes, I used Table 2 (those are default settings in BRL). See above comment
	
My Figure 1 is equivalent to Sadinle's figure 3. He compares against Fellegi Sunter, whereas I compare against BRL. I found his style of graphic difficult to read, especially because I wanted to show F-measure in addition to recall and precision. 

Sadinle's Figure 4 just shows the results when you add the option of not declaring matching status (leaving it undecided by the model). This didn't seem to be so relevant in my paper, so I didn't have it before. I just completed the simulation, and I'll include it under a seperate section.
}
...
NOTE: Sadinle only shows results for the 10\% overlap case. Should I inlcude all of this?
NOTE: When I ran this, I didn't realize I needed to calculate Rejection Rate. I'll have to run the simulation again to get this. 
}
...
{
\color{blue} For Beka: I'm not using any blocking for either method.

Some of the hashing techniques could work for BRL (updating the m and u parameters), and they technically could be used for updating Z, but it would be much less useful because BRL requires knowing which exact record you match to, not just the agreement pattern for that record. Its not feasible for me try making those changes since its coded in C.

I'll work out the computational complexity. That can be one of the things we nail down on Thursday!}
...
\textcolor{blue}{This is with post processing already. All I meant here was that is that small tasks require less information. If you do a $500 \times 500$ linkage task and you only have first and last name (for example), thats likely enough to get all the correct matches. However, once you do $20000 \times 20000$, its more likely that more than one person in the second file has the same first and last name, so you need more information to distinguish. I mentioned before that the simulated files actually have 6 fields of information, but I only used 4 originally because thats what Sadinle did. I can use all 6 though to see what happens?}
 ...
 \textcolor{blue}{When I use the compareRecords from BRL to do the pairwise comparison on the concatenated files, its too  big for R to handle! One could do hashing, reduce the data down to summary statistics, and alter the BRL to use that new object, but that would be a whole different project!}
...
thresholds provided in Table~\ref{Tab:el_salvador_cutoffs_1}. This took 422 seconds for our proposed
