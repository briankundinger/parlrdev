...
\title{Efficient and Scalable Entity Resolution with Fast Beta Linkage  (fabl)}
...
\date{
  ...
  \textsuperscript{b}Department of Statistical Science and Computer Science, Duke University\\Principal Mathematical Statistician, United States Census Bureau\\
  \today}
...
Record linkage is straightforward when provided unique identifiers or highly reliable
information, but becomes difficult when data sources are plagued with
error. This motivates the use of probabilistic record linkage techniques that are able to model the reliability of the information and which are more robust to errors than are deterministic methods. Unfortunately, probabilistic record linkage is computationally intense, and many entity resolution tasks are large in nature,
motivating the search for scalable and efficient record linkage techniques. 

In this paper, we propose an extention to the \emph{Beta Record Linkage}
(\texttt{BRL}) method proposed proposed by \citep{sadinle2017} for this very end.  We increase the speed of the \texttt{BRL} procedure through the modification to the model specification that allows for parallel computing of the linkage parameter, and use hashing techniques to hasten calculations and reduce computational complexity. Additionally, we introduce storage efficient indexing (SEI), and a data partitioning approach to make the method amenable to large linkage tasks. To recognize the lineage from the original \texttt{BRL} method, we name our method \emph{Fast Beta Linkage} (\texttt{fabl},
pronounced ``fable'')

In what follows, Section 2 describes previous work in record linkage that lays the foundation for our method, Section
3 provides our model and the Gibbs sampler used for posterior inference,
and Section 4 describes strategies used for more efficient and scalable
implementation. Then, Section 5 demonstrates the accuracy and speed of
out method through two simulation studies, and Section 6 reveals
interesting properties of the method through a case study of El
Salvadoran casualty records. Lastly, our discussion highlights open
...
model. Jaro (1989) provided guidelines for appropriate binning for the
...
Jaro devised an procedure to reduce a set of conflicting matches to the
mostly likely set of one-to-one matchings, the abundance of false
matches can bias the estimation of other parameters of the model.

\paragraph{Sadinle's Method for Bipartite Matching} \texttt{BRL} uses the same comparison vector approach as Fellegi and Sunter, but proposed a prior distribution for the set of matches that strictly enforced one-to-one matching without the need for any post-hoc processing procedures. Specifically, in each iteration of his Gibbs sampler, he considers each record \(j\in B\), removes from consideration the records \(i\in A\) that have
already been matched, and then samples a potential link. However,
...
\paragraph{Deterministic Blocking}

\paragraph{Other Approach (Will probably remove)}Outside of the Fellegi-Sunter framework, some researchers have developed
...
each \(Z_j\), creating a sampler that is \emph{perfectly
	parallelizable}, allowing for significant computational gains. More
...
matched to at most one record in \(A\), but allows for the possiblity
that multiple records in \(B\) match to the same record in \(A\). We
...
For a more indepth explanation of this function and the induced Bayes
estimate, see Sadinle 2017.
...
posterior probability. Such a resolution procedure indeed is equivalent
...
computations, we instead represent the comparison as a concatenation of
\(F\) many binary indicator vectors of lengths \(L_f\). For example, if
...
these comparison vectors themselves, but instead only a hashed value
...
Laslty, for large data, we can partition the two datasets \(A\) and
...
computational simple even for large sets of candidate records, but this
...
find it useful to have a single metric to gauge overall performance, and
thus also include F measure.
...
\begin{figure}[ht]
	...
	{\centering 
		...
		\includegraphics{../notes/figures/sadinle_sim_plot} 
	}
...
scores across first name, last name, and day, month. For simplicity, we
consider only exact matching, so a vector (1, 0) corresponds to
agreeement and (0, 1) to disagreement. We simulate these data for
...
\begin{figure}[ht]
	...
	{\centering \includegraphics{../notes/figures/sadinle_speed_plot2} 
		
	}
	
	\caption{Runtime for BRL and fabl to run 1000 Gibbs iterations, including hashing step for fabl, for increasing values of both $n_A$ and $n_B$}\label{fig:speed1}
\end{figure}
...
\begin{figure}[t]
	...
	{\centering \includegraphics{../notes/figures/speed_plot_fixed_nB} 
		
	}
	
	\caption{Runtime for BRL and fabl to run 1000 Gibbs iterations, including hashing step for fabl, with increasing $n_A$, and $n_B$ fixed at 500}\label{fig:speed2}
\end{figure}
...
\begin{figure}[t]
	...
	{\centering \includegraphics{../notes/figures/el_salvador/m_posterior_smallP} 
...
aggreement levels creates unnecesary distictions in the data and makes
...
number of unique agreement patterns, and how significant computional
...
\(\mathbf{u}\) paramters under each method are very similar, as shown in
Figure~\ref{fig:m-and-u}.

\subsection{Violations of One-to-One Matching}
\label{violations-of-one-to-one-matching}
...
\begin{figure}[t]
	...
	{\centering \includegraphics{../notes/figures/el_salvador/overlap_distribution_smallP_bayes} 
...
resolving matches that violated one-to-one requirements, yeilded 180
...
\begin{figure}[t]
	...
	{\centering \includegraphics{../notes/figures/el_salvador/rosa_records} 
...
\begin{figure}[t]
	...
	{\centering \includegraphics{../notes/figures/el_salvador/bad_mixing} 
...
Figure XX shows the values that \(Z_{825}\) and \(Z_{826}\) take on
...
linkage when modelling assumptions are not met in practice. We explored
...
\texttt{fabl} for use in these scenarious is one potential avenue for
future work.
