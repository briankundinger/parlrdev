...
\paragraph{Other Approaches (Will probably remove)}Outside of the Fellegi-Sunter framework, some researchers have developed
methods that model the data directly, not a derived set of comparison
vectors. As an early example,  produced
...
patterns rather than the \(n_A \times n_B\) record pairs. Some of these techniques were used by \citep{enamorado2019} to produce \texttt{fastlink} a fast a scalable implementation of the Fellegi-Sunter model, and others have been created here for our particular context. 
...
but this representation will ease calculations and posterior updates. This is also the form the data takes in the \texttt{BRL} package in \texttt{R}. 
...
\begin{figure}[ht]
	...
	\caption{Run-time for BRL and fabl to run 1000 Gibbs iterations, including hashing step for fabl, for increasing values of both $n_A$ and $n_B$}\label{fig:speed1}
\end{figure}
...
\begin{figure}[t]
	...
	\caption{Run-time for BRL and fabl to run 1000 Gibbs iterations, including hashing step for fabl, with increasing $n_A$, and $n_B$ fixed at 500}\label{fig:speed2}
\end{figure}
...
Left over from dblink paper, might remove.
\end{center}
