...
n_1 + j \quad n_1 + j \quad \text{if records}\;  j \in \bm{X}_2 \; \text{does not have a match in database}\; \bm{X}_1. \\
\end{cases}
\end{align}
...
I think this should be reviewed so that the paper is more clear. Consider ordered record pairs $\bm{X}_1 \times \bm{X}_2.$ 
...
\end{align}
where $p$ is the proportion of record pairs that match. Note that $\bm{m}$ and $\bm{u}$ are vectors of the match/non-match parameters. One common way to estimate the parameters of the aforementioned model is using the EM algorithm. 


\subsection{Pros and Cons of the Fellegi Sunter Method}

\textcolor{red}{Notes below}

``Even if one implores mixture models with a one-to-one constraint, the Fellegi-Sunter decision rule alone may still lead to many-to-many assignments such as the work by Jaro. This led to Sadinle (2017)  incorporating this constraint into the model (as in Fortini et al., 2001, 2002; Liseo and Tancredi, 2011; Tancredi and Liseo, 2011; Larsen, 2002, 2005, 2010, 2012; Gutman et al., 2013) rather than forcing it in a post-processing step."

On the other hand, this greatly adds to the computational complexity as explored by Ted in [CITE]. Explain his paper. 

In this paper, we bridge the gap between both methods, in order to propose a method that takes into account the one-to-one matching constraint, while scaling to large databases.

Next, review briefly the two approach and the important technical components we utilize for our paper.
...
%n_1 + j \quad \text{if records}\;  j \in \bm{X}_2 \; \text{does not have a match in database}\; \bm{X}_1 \\

%describes previous work in record linkage that lays the foundation for our method, Section~\ref{sec:model}
...
\label{sec:related}
In this section, we review work that is most related to our proposed work. For surveys of the record linkage literature, see \citep{christen_2012, Herzog_2007, liseo_2013, Winkler2014, Christophides2019, Jurek2019}.
