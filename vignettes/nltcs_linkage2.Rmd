...
# 
# pattern_df <- df89 %>%
#   unite(pattern, 1:dim(df82)[2], sep = "") %>% 
#   unique() %>% 
#   unlist()
# 
# counts <- df89 %>%
...
test <- HashRecords(df89_partition[[1]])
```
...
counts_per_rec_big <- patterns_hashed %>%
  ...
  comparisons <- list(indicators = NULL,
                      n1 = dim(df82)[1],
                      n2 = dim(df89)[1],
                      nDisagLevs = rep(2, length(fields)),
                      Ztrue = NULL)
...
```{r load_hash}
#saveRDS(cd, file = "../data/ntlcs_hash2.rds")
cd <- readRDS(file = "../data/ntlcs_hash2.rds")
```
...
png(file="../notes/figures/nltcs/postprob_hist2.png")
hist(post_probs)
dev.off()
...
ggsave("../notes/figures/nltcs/overlap_posterior2.png")
```
...
#ggsave("../notes/figures/nltcs/m_posterior.png")
