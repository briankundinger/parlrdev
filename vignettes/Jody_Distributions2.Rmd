...
linkage_cluster <- unlist(map(1:K, ~rep(.x, n2_vec[.x])))
...
LC_samps_split <- array(NA, c(iter, 3, K))
...
for(i in 1:iter){
  ...
  Ztrue <- comparisons[[5]]
  Ztrue_split <- split(Ztrue, linkage_cluster)
  
  Zchain <- BRACS_BK2(comparisons, m_prior, u_prior, alpha, beta, S, burn)
  Zhat <- BRL::linkRecords(Zchain, n1, 1, 1, 2, Inf)
  eval_lc <- GetEvaluations(Zhat, Ztrue, n1)
  LC_samps[i, ] <- eval_lc
  
  Zhat_split <- split(Zhat, linkage_cluster)
  eval_lc_split <- purrr::map2(Zhat_split, Ztrue_split, 
                                .f = ~GetEvaluations(.x, .y, n1))
  for(k in 1:K){
    LC_samps_split[i,,k] <- eval_lc_split[[k]]
  }
...
df_jody <- rbind(LC_df, NLC_df)
results_df_jody <- df_jody %>% 
...
results_df_jody
# current_dist_table <- results_df_jody %>% 
#   kable(digits = 3,
#         caption = "Results for Current Distributions") %>% 
#   kable_classic() %>% 
#   kable_styling(full_width = F) %>% 
#   as_image(file = "../notes/figures/current_dist_table.png")
...
```{r}
LC_df <- data.frame(LC_samps_split[,,2], "LC")
names(LC_df) <- c("recall", "precision", "fmeasure", "method")
NLC_df <- data.frame(NLC_samps_split[,,2], "NLC")
names(NLC_df) <- c("recall", "precision", "fmeasure", "method")

df_bk <- rbind(LC_df, NLC_df)
results_df_women <- df_bk %>% 
  pivot_longer(cols = 1:3, names_to = "metric") %>% 
  group_by(method, metric) %>% 
  summarize(avg = mean(value),
            lower = quantile(value, .2),
...
results_df_women
simple_dist_table_women <- results_df_women %>%
  kable(digits = 3,
        caption = "Results for Simpler Distributions (Women)") %>%
  kable_classic() %>%
  kable_styling(full_width = F) %>%
  #as_image(file = "../notes/figures/simple_dist_table_women.png")
...
```{r}
LC_df <- data.frame(LC_samps_split[,,1], "LC")
names(LC_df) <- c("recall", "precision", "fmeasure", "method")
NLC_df <- data.frame(NLC_samps_split[,,1], "NLC")
names(NLC_df) <- c("recall", "precision", "fmeasure", "method")

df_bk <- rbind(LC_df, NLC_df)
results_df_men <- df_bk %>% 
  pivot_longer(cols = 1:3, names_to = "metric") %>% 
  group_by(method, metric) %>% 
  summarize(avg = mean(value),
            lower = quantile(value, .2),
...
results_df_men
simple_dist_table_women <- results_df_men %>%
  kable(digits = 3,
        caption = "Results for Simpler Distributions (Men)") %>%
  kable_classic() %>%
  kable_styling(full_width = F) %>%
  as_image(file = "../notes/figures/simple_dist_table_men.png")
