---
title: "elsalvador_small_P"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{elsalvador_small_P}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>"
)
```

```{r setup}
devtools::load_all()
library(parlrdev)
library(tidyverse)
library(knitr)
library(kableExtra)
```

```{r}
rm(list=ls())
set.seed(2)
R <- 10
show_progress <- T
fast <- F

cd <- readRDS("../data/comparison_elsalvador_smallP.rds")
nA <- cd$n1
nB <- cd$n2
levels <- cd[[4]]
P <- prod(levels) 
var_names <- cd$compFields[,1]
ptm <- proc.time()
Zchain_parlr <- BKSimple_hash2(cd, R = R, show_progress = T)
elapsed_parlr <- proc.time() - ptm
Zhat_parlr <- LinkRecordsBK(Zchain_parlr[[1]], nA, 1, 1, 2, Inf)
overlap_parlr <- sum(Zhat_parlr[[1]] < nA + 1)
results_parlr <- c(overlap_parlr, elapsed_parlr[3])

Zhat_resolved <- ResolveConflicts(Zhat_parlr)
overlap_resolved <- sum(Zhat_resolved < nA + 1)
overlap_samples_parlr <- apply(Zchain_parlr[[1]], 2, function(x){
  sum(x < nA + 1)
})
not_one2one_samps <- apply(Zchain_parlr[[1]], 2, function(x){
  sum(duplicated(x[x < nA + 1]))
})
results_parlr_resolved <- c(overlap_resolved, elapsed_parlr[3])


ptm <- proc.time()
Zchain_sadinle <- BRL::bipartiteGibbs(cd)
elapsed_sadinle <- proc.time() - ptm
Zhat_sadinle <- LinkRecordsBK(Zchain_sadinle[[1]], nA, 1, 1, 2, Inf)
overlap_sadinle <- sum(Zhat_sadinle[[1]] < nA +1)

overlap_samples_sadinle <- apply(Zchain_sadinle[[1]], 2, function(x){
  sum(x < nA + 1)
})[-(1:100)]

results_sadinle <- c(overlap_sadinle, elapsed_sadinle[3])

results_table <- rbind(results_parlr,
                       results_parlr_resolved,
                       results_sadinle)
colnames(results_table) <- c("Overlap", "time (sec)")
rownames(results_table) <- c("parlr", "parlr_resolved",
                             "sadinle")
results_table %>% 
  kable(caption = paste("Linkage Results with P = ", paste(P), "patterns"), digits = 3) %>% 
  kable_classic() %>% 
  kable_styling(full_width = F) %>% 
  as_image(file = "../notes/figures/el_salvador/time_table_small_P.png")


```


```{r}
method <- c(rep("parlr", 900), rep("sadinle", 900))
overlap <- c(overlap_samples_parlr, overlap_samples_sadinle)
df <- data.frame(overlap, method)

df %>% 
  ggplot(aes(y = overlap, fill = method)) +
  geom_histogram() + 
  labs(y = "Overlap",
       x = NULL,
       title = "Posterior Distribution of Overlap",
       color = "Method") +
  coord_flip() + 
  theme_bw() + 
  scale_fill_brewer(palette="Set1")

ggsave("../notes/figures/el_salvador/overlap_distribution_smallP.png")
```

```{r}
#samps <- Zchain_parlr$m
ProcessChain <- function(samps){
  avg <- apply(samps, 1, mean)
  upper <- apply(samps, 1, function(x){
    quantile(x, .975)
  })
  lower <- apply(samps, 1, function(x){
    quantile(x, .025)
  })
  params <- factor(unlist(sapply(levels, seq_len)))
  field <- factor(unlist(mapply(function(x, y) {
    rep(x, y)
  }, x = var_names, y = levels)), levels = var_names)
  
  data.frame(params, field, avg, upper, lower)
}
m_parlr <- ProcessChain(Zchain_parlr$m)
m_parlr$method <- "parlr"
m_sadinle <- ProcessChain(Zchain_sadinle$m)
m_sadinle$method <- "sadinle"
df <- rbind(m_parlr, m_sadinle)
df %>% 
  ggplot(aes(x = params, y = avg,
             ymin = lower, ymax = upper, color = method)) +
  geom_pointrange(position = position_dodge2(width = .5)) +
  facet_wrap(~field, scales = "free_x") + 
  labs(title = paste("Distributions of m parameters, P =", paste(P), "Patterns"),
       y = "Probability",
       x = "Agreement Level",
       color = "Method") + 
  theme_bw() + 
  scale_color_brewer(palette="Set1")

ggsave("../notes/figures/el_salvador/m_posterior_smallP.png")


```

## Comparing Mixing

```{r}
find <- which(Zhat_parlr[[1]] == 2776)
probs <- Zhat_parlr[[2]][find]
dups_parlr <- Zchain_parlr[[1]][find,]
dups_sadinle <- Zchain_sadinle[[1]][find, ]
dups_sadinle[dups_sadinle > nA + 2] <- nA + 1


dups_parlr <- cbind(dups_parlr, c("Rosa", "Rosa Maria") , "parlr")
dups_sadinle <- cbind(dups_sadinle, c("Rosa", "Rosa Maria") , "sadinle")
df <- data.frame(rbind(dups_parlr, dups_sadinle[,-(1:100)]))
names(df)[c(901, 902)] <- c("record", "method")
df_mixing <- df %>% 
  pivot_longer(1:900) %>% 
  group_by(record, method) %>% 
  mutate(iter = row_number()) %>% 
  filter(iter < 400) %>% 
  mutate(type_match = case_when(
    value == 2776 ~ "Correct Match",
    value > nA ~ "No match",
    TRUE  ~ "Match to other record"
  ))




df_mixing %>% 
  ggplot(aes(x = iter, y = value, color = type_match)) +
  geom_point(size = .8) +
  facet_grid(record~method) + 
  #scale_x_continuous(labels = NULL, breaks = NULL) + 
  labs(title = "Near Matches under Parlr and Sadinle",
       subtitle = "Both have undesirable behavior",
       x = "Gibbs Iterations",
       y = expression(Z[j]),
       color = "Type of Match") +
  theme_bw(base_size = 12)

ggsave("../notes/figures/el_salvador/bad_mixing.png")
```


