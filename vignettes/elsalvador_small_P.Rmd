...
#Zhat_empirical <- LinkRecordsBK(final_gibbs, nA, 1, 1, 2, Inf)
Zhat_fabl_partial <- LinkRecordsBK(Zchain_fabl[[1]], nA, 1, 1, 2, .1)

overlap_fabl <- sum(Zhat_fabl[[1]] < nA + 1)
#overlap_empirical <- sum(Zhat_empirical[[1]] < nA + 1)
...
#Zhat_resolved_emp <- ResolveConflicts(Zhat_empirical)
#overlap_resolved <- sum(Zhat_resolved_emp < nA + 1)
...
overlap_samples_fabl_resolved <- overlap_samples_fabl - not_one2one_samps
credint_fabl_overlap <- quantile(overlap_samples_fabl_resolved, c(.025, .975))
...
bayes_df <- data.frame(bayes, method)[c(2, 1), ]
#bayes_df <- bayes_df[c(2, 3), ]

method <- c(rep("fabl", S), rep("BRL", S))
overlap <- c(overlap_samples_fabl_resolved, overlap_samples_sadinle)
#iteration <- c(iter_count, iter_count)
df <- data.frame(overlap, method)
#saveRDS(df, "../data/sadinle_smallp_df")
df <- readRDS( "../data/sadinle_smallp_df")
df <- df %>% 
  group_by(method) %>% 
...
df %>% 
  ...
  geom_histogram(position = 'identity', alpha = .9, binwidth = 5) + 
...
avg_violation <- mean(not_one2one_samps)
...
not_one2one_samps %>% 
  ...
  geom_hline(yintercept = avg_violation) +
  labs(y = NULL,
       x = NULL) +
  coord_flip() +
  theme_bw()

ggsave("../notes/figures/el_salvador/violations_hist.png")
...
overlap_trace
ggsave("../notes/figures/el_salvador/overlap_trace.png", plot = overlap_trace, width = 7, height = 7)
...
ggsave("../notes/figures/el_salvador/pi_trace.png", plot = pi_trace)
```
...
```{r}
thing <- (Zhat_fabl[[2]] >nA ) &  (Zhat_sadinle[[1]] < nA  +1)
match1_index <- which(thing)
match2_index <- Zhat_sadinle[[1]][match1_index]
...
cbind(Zhat_fabl[[2]][thing], Zhat_sadinle[[2]][thing])
```
```{r}
CDHES[match2_index, ]
```
