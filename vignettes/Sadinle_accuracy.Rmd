...
# cd[[4]] <- c(4, 4, 2, 2)
  ...
  # bouncing_matches <- apply(Zchain[[1]][, (S- lag -1):(S-1)],1, function(x){
  #   x[x<nA + 1] %>%
  #   unique() %>%
  #   length()
  # })
  # table(bouncing_matches)
  # 
  # ptm <- proc.time()
  # Zchain_d <- BKSimple_hash_discount(cd)
  # elapsed <- proc.time() - ptm
  # Zhat <- LinkRecordsBK(Zchain[[1]], n1, 1, 1, 2, Inf)
  # eval <- GetEvaluations(Zhat[[1]], Ztrue, n1)
  # 
  #   bouncing_matches_d <- apply(Zchain_d[[1]][, (S- lag -1):(S-1)],1, function(x){
  #   x[x<nA + 1] %>%
  #   unique() %>%
  #   length()
  # })
  #   table(bouncing_matches_d)
...
sad_samps2 <- rbind(sad_acc_samps2[,,1], sad_acc_samps2[,,2], sad_acc_samps2[,,3])
error_level <- rep(rep(c("One Error", "Two Errors", "Three Errors"), each = 100), 3)
error_level <- factor(error_level, levels = c("One Error", "Two Errors", "Three Errors"))
#error_level <- 3
# error_level <- factor(error_level, levels = c("One Error", "Two Errors", "Three Errors"))
...
names(fabl_df2) <- c("NPV", "PPV", "Fmeasure", "RR", "time", "overlap", "error_level", "method")
# fabl_resolved_df <- data.frame(fabl_samps_resolved, error_level, "fabl_resolved")
...
names(sadinle_df2) <- c("NPV", "PPV", "Fmeasure", "RR", "time", "overlap", "error_level",  "method")
...
results_df2 <- df2 %>%
  ...
  summarize(avg = mean(value, na.rm = T),
            ...
            .groups = "drop") %>% 
  mutate(metric = factor(metric, c("NPV", "PPV", "RR")),
         overlap = case_when(
           overlap == 50 ~ "10% Overlap",
           overlap == 250 ~ "50% Overlap",
           overlap == 450 ~ "90% Overlap"
...
saveRDS(results_df2, file = "Sadinle_Replication_Results_Partial_new")
...
# ggsave(filename = "../notes/figures/sadinle_sim_plot_partial_new.png",
#        plot = acc_plot_partial)


results_df2_DR <- df2 %>%
  mutate(DR = 1 - RR) %>% 
  pivot_longer(cols = c(1:4, 9), names_to = "metric") %>%
  filter(metric != "Fmeasure", 
         metric != "RR") %>% 
  group_by(method, metric, error_level, overlap) %>%
  summarize(avg = mean(value, na.rm = T),
...
saveRDS(results_df2, file = "Sadinle_Replication_Results_Partial_DR")

results_df2_DR$method[results_df2_DR$method == "Sadinle17"] <- "BRL"

acc_plot_partial_DR <- results_df2_DR %>% 
  ggplot(aes(x = metric, y = avg,
             ymin  = lower, ymax = upper,
             color = method)) +
  geom_pointrange(position = position_dodge2(width = .5)) + 
...
ggsave(filename = "../notes/figures/sadinle_sim_plot_partial_DR.png",
       plot = acc_plot_partial_DR)
```
...
<!-- candidates <- 1:nA -->
<!-- ptm <- proc.time() -->
<!-- task <- sapply(1:nB, function(x){ -->
<!--   result <- thing %in% candidates -->
<!-- }) -->
