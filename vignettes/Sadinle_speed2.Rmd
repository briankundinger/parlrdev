...
alpha <- beta <- 1
results_bk_samps <- matrix(NA, nrow = length(n1_vals), ncol = 5)
results_sad_samps <- matrix(NA, nrow = length(n1_vals), ncol = 5)
results_bk_samps_resolved <- matrix(NA, nrow = length(n1_vals), ncol = 5)
...
for(i in seq_along(n1_vals)){
  ...
  #parlr method
  ptm <- proc.time()
  #Zchain <- BKSimple_hash2_pat(cd, R = R)
  Zchain <- BKSimple_hash2(cd, all_patterns = all_patterns)
  hash_time <- Zchain[[4]]
  #Zchain <- FellegiSunter(cd)
  Zchain <- Zchain[[1]]
  ...
  results_bk_samps[i, ] <- c(eval_simple, elapsed[3], hash_time)
  ...
  results_bk_samps_resolved[i, ] <- c(eval_resolved, elapsed[3], hash_time)
  ...
  results_sad_samps[i,] <- c(eval, elapsed[3], hash_time)
  gc()
  print(i)
}
...
parlr_df <- data.frame(results_bk_samps, n2_vals, "fabl")
names(parlr_df) <- c("Recall", "Precision", "Fmeasure", "time", "hash_time",  "n", "method")

parlr_resolved_df <- data.frame(results_bk_samps_resolved, n2_vals, "fabl_resolved")
names(parlr_resolved_df) <- c("Recall", "Precision", "Fmeasure", "time", "hash_time", "n", "method")

sadinle_df <- data.frame(results_sad_samps, n2_vals, "BRL")
names(sadinle_df) <- c("Recall", "Precision", "Fmeasure", "time", "hash_time", "n",  "method")

df<- rbind(parlr_df, parlr_resolved_df, sadinle_df)

speed_plot <- df %>% 
  filter(method != "fabl_resolved") %>% 
  ggplot(aes(x = n, y = time, color = method)) +
  geom_line(size = 1) +
  labs(x = expression(n[1] *","* n[2]),
       y = "Time (seconds)",
       color = "Method") + 
...
ggsave(filename = "../notes/figures/sadinle_speed_plot_slides2.png",
       plot = speed_plot)
...
as_image(file = "../notes/figures/simple_dist_table2.png")
```
