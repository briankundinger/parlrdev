...
%
%\begin{aug}
%\author{\fnms{Neil G.} \snm{Marchant}\thanksref{addr1}\ead[label=e1]{nmarchant@unimelb.edu.au}},
%\author{\fnms{Benjamin I. P.} \snm{Rubinstein}\thanksref{addr1}\ead[label=e2]{brubinstein@unimelb.edu.au}}
%\and
%\author{\fnms{Rebecca C.} \snm{Steorts}\thanksref{addr2}\ead[label=e3]{beka@stat.duke.edu}}
%
%\runauthor{N. G. Marchant et al.}
%
%\address[addr1]{School of Computing and Information Systems,
...
\author{\fnms{Jerome} \snm{Reiter}\thanksref{addr1}\ead[label=e2]{jerry@stat.duke.edu}}
\and
\author{\fnms{Rebecca C.} \snm{Steorts}\thanksref{addr2}\ead[label=e3]{beka@stat.duke.edu}}

\runauthor{Brian Kundinger et al.}
...
%%% ** Keywords **
%\begin{keyword}%[class=MSC]
%bipartite record linkage, Bayesian methods, Gibbs sampling, hashing techniques, Markov chain Monte carlo, parallel/distributed computing
%%\kwd{}
%%\kwd[]{}
%\end{keyword}
...
\end{align}
where $p$ is the proportion of records that match, and $\bm{m}$ and $\bm{u}$ are vectors of parameters for the matching and nonmatching record pairs. These unknown parameters are often estimated using the EM algorithm \citep{Winkler1988}. 
...
Importantly, this independence means that we have relaxed the one-to-one requirement from \texttt{BRL}; our sampler does ensure that each record in \(\bm{X}_2\) can be matched to at most one record in \(\bm{X}_1\), but allows for the possibility that multiple records in \(\bm{X}_2\) match to the same record in \(\bm{X}_1\). This behavior may be desirable in its own right; \cite{Newcombe1963} was interested in matching birth records to marriage certificates, a situation in which such ``many-to-one'' matchings are reasonable. In situations when a bipartite estimate is desired, we can use a simple post processing procedure shown in  section~\ref{bayes-estimate} to resolve illegal matchings.  
...
\begin{align*}
	...
	&\implies Z_j^{(s+1)} | \Phi, \Gamma, \pi \propto \begin{cases} 
